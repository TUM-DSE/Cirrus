...
def main():
    ...
    for container in ax.containers:
        ax.bar_label(container, fmt = "%.1f", fontsize=FONTSIZE)
    # set hatch
    bars = ax.patches
    hs = []
    for h in hatches:
    ...
    # sns.despine()
    plt.tight_layout()
    plt.savefig(os.path.join(result_dir, "perf.pdf"), format="pdf", pad_inches=0, bbox_inches="tight")
